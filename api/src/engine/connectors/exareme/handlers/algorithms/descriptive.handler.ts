...
export default class DescriptiveHandler extends BaseHandler {
  ...
  static readonly descriptiveToTable = (
    ...
    const datasetEnumMapping = domain.variables.find(v => v.id === 'dataset')?.enumerations?.reduce((map, e) => {
      map[e.value] = e.label;
      return map;
    }, {} as Record<string, string>) || {};

    const datasets: string[] = Array.from(new Set(stats.map((d) => datasetEnumMapping[d.dataset] || d.dataset)));
    const variables: string[] = Array.from(new Set(stats.map((d) => d.variable)));

    const columns = (variable: string) => {
      ...
      const variableObj = domain.variables.find((v) => v.id === variable);
      const enumMapping =
          variableObj?.enumerations?.reduce((map, e) => {
            map[e.value] = e.label;
            return map;
          }, {} as Record<string, string>) || {};

      const mapCodeToLabel = (code: string) => enumMapping[code] || code;
