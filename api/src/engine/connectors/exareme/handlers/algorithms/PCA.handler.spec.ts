  ...
  it('Test PCA handler with regular data (no edge cases)', () => {
    ...
    exp.results.forEach((result) => {
      if (result['matrix']) {
        ...
        const matrix = data.eigenvectors[0].map(
          (_, i) => data.eigenvectors.map((row) => row[i]), // reverse matrix as we want row-major order
        );
      ...
      if (result['barValues']) {
        ...
        expect(barchart.barValues).toEqual(data.eigenvalues);
      }
    });
  });
});
